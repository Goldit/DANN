...
LAMBDA             = 10.0
COLOR_DISTORT      = True
PREPROCESSING_MODE = 'mean' # mean or norm
...
def pre_process_image_norm(image, training):
...
def pre_process_image(image, training):
    if PREPROCESSING_MODE == 'mean':
        return pre_process_image_mean(image, training)
    else:
        return pre_process_image_norm(image, training)
...
def cnn_model_mnist_fn(features, labels, mode):
    ...
    # FC Layers
    all_features =tf.contrib.layers.flatten(pool2)
    ...
    lb_fc1   = tf.layers.dense(inputs=classify_feats, units=100, activation=tf.nn.relu, name="lb_fc1")
    lb_fc2   = tf.layers.dense(inputs=lb_fc1, units=100, activation=tf.nn.relu, name="lb_fc2")
    label_logits = tf.layers.dense(inputs=lb_fc2, units=NUM_CLASSES, name="lb_softmax")
    ...
    if mode == learn.ModeKeys.TRAIN:
        ...
        dm_fc1   = tf.layers.dense(inputs=flipped_features, units=100, activation=tf.nn.relu, name="dm_fc1")
        domain_logits = tf.layers.dense(inputs=dm_fc1, units=NUM_DOMAINES, name="dm_softmax")
...
toppad_classifier = learn.Estimator(model_fn=cnn_model_mnist_fn, model_dir="log/{}_{}_{}_{}".format(MODEL_NAME, OPTIMIZER, LEARNING_RATE, PREPROCESSING_MODE))
